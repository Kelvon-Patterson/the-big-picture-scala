...
//2 Write a Scala program to get the character (Unicode code point) at the index within the String.
...
println(uniCodePoint("w3resource - Scala", 9))
  ...
  //3 Write a Scala program to compare two string lexicographically
...
// 15 Write a scala program to find the maximum occurring char in a string
//TODO redo this problem with no help
...
//17. Write a Scala program to count and print all the duplicates in the input string.
  ...
  def countDuplicates(str1: String): Unit ={
    val MAX_CHARS = 256;
...
def stringRotated(str1:String, str2: String): Boolean={
 (str1.length == str2.length) && ((str1+str1).indexOf(str2) != -1)
  ...
  println("These strings are rotations of each other: " + stringRotated("ABACD", "CDABA"))
...
//
  ...
  //TODO:: scala-exercises.org/std_lib/higher_order_functions
//  def res(): Unit = {
